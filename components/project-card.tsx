...
export function ProjectCard({ name, description, projectLink, githubLink, internalLink, skills }: ProjectCardProps) {
   return (
      <div className="relative mt-6 isolate group/project-card">
         <h3 className="typo-h3 relative -z-10 mb-1 underline-offset-4 group-hover/project-card:underline group-hover/project-card:text-green-800 group-hover/project-card:dark:text-green-300">
            {name}
         </h3>
         {skills && (
         ...
         <Link href={internalLink} className="link text-gray-600 dark:text-gray-400 text-sm ml-3">
            Learn more
         </Link>
         <Link href={internalLink} className="absolute inset-0 text-[0px] -z-1">
            Learn more
         </Link>
      </div>
   );
